            ...
            <table>
                    ...
                    <?php
                        ...
                        foreach ($submissoes as $item) {
                            ...
                            <td>'.($item['pathResultado'] == "" ? (isset(session('user')['id']) == $item['id_usuario_envio'] ? '<a href="uploadResult/'.$item['id'].'"><img src="/imgs/upload.svg"/></a>' : "Sem resultado") : (isset(session('user')['nivel']) == 1 ? '<a href="'.$item['pathResultado'].'" download="'.$item['pathResultado'].'"><img src="/imgs/download.svg"/></a> <a href="/cancelUploadResult/'.$item['id'].'"><img src="/imgs/cancel.svg"/></a>' : '<a href="'.$item['pathResultado'].'" download="'.$item['pathResultado'].'"><img src="/imgs/download.svg"/></a>')).'</td>
                            </tr>';
                        }
                    ?>
            </table>
